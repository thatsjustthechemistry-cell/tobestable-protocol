...
- [ ] **arm_floor authority gate — devnet-prove before mainnet.** The H1 authority gate has CI-verified unit tests for the arming *math* (`pyth_math_tests::arm_gate_*`), but the *authorization* rejection can't run under `anchor test` (ArmFloor needs real Raydium vaults + a Pyth PriceUpdateV2). Prove it once on devnet:
  ```bash
  # 1. deploy CURRENT main to a throwaway devnet program (the legacy devnet
  #    program predates the H1 fix and would prove nothing), initialize it,
  #    and run set_pool_config so the pool is configured
  # 2. then:
  node scripts/prove-arm-floor-authority.js --program <THROWAWAY_DEVNET_PROGRAM>
  ```
  Signs `arm_floor` with a freshly generated throwaway key (safe by construction — it can never be the authority, so it cannot arm the floor), asserts the program rejects with `Unauthorized`, and asserts `floor_active` is unchanged. Exit 0 = gate held. It refuses to run — rather than reporting a false pass — if the floor is already armed or the pool is unconfigured, since those would mask the rejection. Test-side runbook also in `tests/neco_token.ts` (§9b).
