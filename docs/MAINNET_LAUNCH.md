...
- [x] **Council key isolation — ✅ CONFIRMED.** The 3 council keys are held by 3 different people on the team, on separate machines (Backpack / Phantom / Solflare, each password-protected). The founder holds one and cannot reach the 2-of-3 threshold alone. ⚠️ Each holder is responsible for backing up their OWN seed phrase — confirm all three have done so before Step 5, since losing 2 of 3 after the authority handoff freezes governance permanently.
...
- [x] **arm_floor authority gate — ✅ COVERED IN CI (2026-07-19), no manual devnet run needed.** The H1 authorization rejection is now a real, running test: CI clones mainnet Raydium + the Pyth receiver + Wormhole into the local validator, creates a genuine TOBE/wSOL pool, posts a fresh Hermes price, and asserts both `Unauthorized` and that `floor_active` stays false (`tests/neco_token.ts` §28). The arming *math* remains unit-tested separately (`pyth_math_tests::arm_gate_*`).
  > The devnet route this checklist previously prescribed is in fact **impossible**: `arm_floor` requires a configured pool, and `set_pool_config` cannot succeed on devnet because the mainnet Raydium program id is compiled in. `scripts/prove-arm-floor-authority.js` is retained only as a mainnet-side spot-check after launch; it is no longer a pre-launch gate.
...
> **Rehearsal note:** the governance mechanics (proposal → 2-of-3 vote → the treasury PDA's CPI-signed execution) were proven end-to-end on devnet — the transaction reached deep inside `set_pool_config`, confirming the authority signer check passes correctly for a real governance-executed call. Raydium pool ownership cannot be tested on **devnet**: the contract's `AccountLoader<PoolState>` validates against the **mainnet** Raydium CPMM program ID (the `raydium-cp-swap` dep is pinned `default-features = false`, so `CPMMoo8L3F4NbTegBCKVNunggL7H1ZpdTHKxQB5qKP1C` is compiled in and devnet's `DRaycpLY…` is not), so a devnet-created pool is rejected by design. That is a security feature (it rejects spoofed pool accounts), not a gap.
>
> ✅ **It IS now covered on localnet (2026-07-19).** CI clones mainnet Raydium into the test validator, creates a genuine TOBE/wSOL CPMM pool, and runs `set_pool_config` against it — verifying every recorded field, the captured floor baseline, and that a non-authority is rejected. See `tests/neco_token.ts` §27 and `tests/raydium-localnet-probe.ts`. This step is no longer untested.
